...
export default function Home() {
  ...
  useEffect(() => {
    async function loadList() {
      ...
      await firebase.database().ref('historico').child(uid).orderByChild('date').equalTo(format(newDate, 'dd/MM/yyyy'))
  ...
  function handleDelete(data) {
    ...
    Alert.alert(
      'Cuidado Atenção!',
      `Você deseja excluir ${data.tipo} - Valor: ${data.valor.toFixed(2)} ? `,
      [
        {
          text: 'Cancelar',
          style: 'cancel'
